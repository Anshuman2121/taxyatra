...
export function HomePage({ onNavigate, refreshTrigger }: HomePageProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      for (const pan of selectedUsers) {
        try {
          const response = await api.deleteUser(pan);
          if (response.success) {
            results.push(pan);
          } else {
            errors.push(`${pan}: ${response.message || 'Unknown error'}`);
          }
        } catch (e: any) {
          // This catches errors like "No handler registered" if main process wasn't restarted
          errors.push(`${pan}: ${e.message || 'Failed to call delete API'}`);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-gold-200 selection:text-gold-900 flex flex-col pt-20 pb-20">
      ...
      <div className="flex-1 container mx-auto px-4 py-6 max-w-[95%] flex flex-col space-y-6">

        {/* Actions & Search Bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-slate-50 z-10 shadow-sm">
                <TableRow className="border-b border-slate-200 hover:bg-slate-50">
                  ...
                  <TableHead className="text-slate-700 font-semibold">Mobile</TableHead>
                ...
                {isLoading ? (
                  ...
                  sortedUsers.map((user, index) => (
                    ...
                    >
                      ...
                      <TableCell className="text-sm text-slate-600">{user.mobileNo || '-'}</TableCell>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          <button onClick={() => onNavigate('add-user')} className="flex flex-col items-center justify-center p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-gold-300 transition-all group">
          ...
          <button onClick={handleDelete} className="flex flex-col items-center justify-center p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-red-300 transition-all group">
          ...
          <button className="flex flex-col items-center justify-center p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-gold-300 transition-all group">
