...
export class DownloadAISTask extends BaseTask {
    ...
    async execute(
        ...
        try {
            ...
            await this.handleDownloadModal(aisPage, event, downloadPath);
    ...
    private async handleDownloadModal(page: Page, event: Electron.IpcMainInvokeEvent, downloadPath: string) {
        // Find JSON button
        await page.evaluate(() => {
        ...
        if (captchaImage) {
            ...
            const captchaText = await new Promise<string>((resolve, reject) => {
                const timeout = setTimeout(() => reject(new Error('Captcha timeout')), 120000);
                ipcMain.once('download:captcha-response', (_: any, response: { text: string; cancelled: boolean }) => {
                    clearTimeout(timeout);
                    if (response.cancelled) reject(new Error('Captcha cancelled'));
    ...
    private async typeCaptcha(page: Page, text: string) {
        ...
        try {
            ...
            const handle = await page.evaluateHandle(() => document.querySelector('input[type="text"]') as Element);
            if (handle.asElement()) captchaInput = handle.asElement();
        }

        if (captchaInput) {
            await captchaInput.click();
            await captchaInput.evaluate(el => (el as HTMLInputElement).value = '');
            await this.wait(500);
