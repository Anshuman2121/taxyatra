...
export function UserProfilePage({ pan, onBack }: UserProfilePageProps) {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            setIsLoading(true);
            try {
                const data = await api.getUserData(pan);
                if (data) {
    ...
    const handleFetchOnline = async () => {
        ...
        api.removeAllFetchProgressListeners();
        api.onFetchProgress((event: any, message: string) => {
    ...
    const handleSave = async () => {
        ...
        if (!validateForm()) {
            ...
            alert('Validation failed! Please fix the errors in the form.');
            return;
        }
        ...
        try {
            ...
            console.log('handleSave: Calling api.updateUserDetails...');
            const result = await api.updateUserDetails(dataToSave);

            if (result.success) {
                console.log('handleSave: Save successful');

                // Also save password if present
                ...
                console.error('handleSave: Save returned failure', result.error);
                alert('❌ Failed to save: ' + (result.error || 'Unknown error'));
            }
        } catch (error: any) {
            console.error("handleSave: Exception caught", error);
    ...
    useEffect(() => {
        ...
        api.onCaptchaRequired(handleCaptchaRequired);

        return () => {
            api.removeAllCaptchaListeners();
        };
    }, []);

    const handleCaptchaSubmit = (captchaText: string) => {
        console.log('✅ Sending captcha response:', captchaText);
        api.sendCaptchaResponse(captchaText, false);
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    const credentials = await api.getPanWithPassword(pan);
                                                    ...
                                                    const result = await api.downloadAIS(pan, credentials.password, selectedFinancialYear);
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    setDownloadStatus('🔐 Logging in...');
                                                    const result = await api.downloadTIS(pan, credentials.password, selectedFinancialYear);
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    setDownloadStatus('🔐 Logging in...');
                                                    const result = await api.download26AS(pan, credentials.password, assessmentYear);
